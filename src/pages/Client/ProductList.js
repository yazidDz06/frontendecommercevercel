...
export default function Produits() {
  ...
  //  handleSearch optimisé avec useCallback
  ...
  //  produits groupés par catégorie avec useMemo
  const produitsParCategorie = useMemo(() => {
    const map = {};
    listeCat.forEach((cat) => {
      map[cat.id] = listeProd.filter((prod) => prod.categoryId === cat.id);
    });
    return map;
  }, [listeProd, listeCat]);
            ...
            {listeCat.map((cat) => (
              <div key={cat.id} className="mb-10 sm:mb-12">
                ...
                <div className="flex md:grid gap-4 overflow-x-auto md:overflow-visible pb-4 md:pb-0 scrollbar-hide 
                  ...
                  {produitsParCategorie[cat.id]?.map((product) => (
                    <ProductCard key={product.id} product={product} onAdd={addToCart} />
                  ))}
                </div>
              </div>
